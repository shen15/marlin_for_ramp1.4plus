...
void calibrate_print_surface( float z_offset )
{
	...
	for( int y = 4; y >= -4; y-- ) {
		...
		// y = 4, -4
		if( abs(y) >= 4 ) {
			g_bed_level[1][y+4] = g_bed_level[2][y+4];
			g_bed_level[7][y+4] = g_bed_level[6][y+4];
		}

		// y = 4, 3, -3, -4
		if( abs(y) >= 3 ) {
			g_bed_level[0][y+4] = g_bed_level[1][y+4];
			g_bed_level[8][y+4] = g_bed_level[7][y+4];
		}

		// Print calibration results for manual frame adjustment.
		for( int x = -4; x <= 4; x++ ) {
			SERIAL_PROTOCOL_F(g_bed_level[x+4][y+4], 3);
			SERIAL_PROTOCOLPGM(" ");
		}
...
//需要添加的代码应该放在这个位置
   case 03:
       digitalWrite(LASER_PIN,HIGH);//Pin指你打算用于控制继电器的接口（arduino Pin编号）。如果跟其它管脚冲突，可能需要配置Pins.h文件中相关管脚为-1.
       break;
   case 05:
...
// Adjust print surface height by linear interpolation over the g_bed_level array.
void adjust_delta( float cartesian[3] )
{
  float grid_x = max(-3.999, min(3.999, cartesian[X_AXIS] / AUTOLEVEL_GRID));
  float grid_y = max(-3.999, min(3.999, cartesian[Y_AXIS] / AUTOLEVEL_GRID));
  int floor_x = floor(grid_x);
  int floor_y = floor(grid_y);
  float ratio_x = grid_x - floor_x;
  float ratio_y = grid_y - floor_y;
  float z1 = g_bed_level[floor_x+4][floor_y+4];
  float z2 = g_bed_level[floor_x+4][floor_y+5];
  float z3 = g_bed_level[floor_x+5][floor_y+4];
  float z4 = g_bed_level[floor_x+5][floor_y+5];
